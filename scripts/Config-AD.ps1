...
New-ADUser -Name $BinduserName1 -GivenName $BinduserName1 -SamAccountName $BinduserName1 -UserPrincipalName ($BinduserName1+"@"+$UPNSuffix) -Path $OUpath.DistinguishedName -AccountPassword(ConvertTo-SecureString $BindPWD -AsPlainText -Force) -Enabled $true
New-ADUser -Name $BinduserName2 -GivenName $BinduserName2 -SamAccountName $BinduserName2 -UserPrincipalName ($BinduserName2+"@"+$UPNSuffix) -Path $OUpath.DistinguishedName -AccountPassword(ConvertTo-SecureString $BindPWD -AsPlainText -Force) -Enabled $true
